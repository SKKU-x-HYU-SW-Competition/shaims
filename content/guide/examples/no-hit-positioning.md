...
지나가버립니다. 그래서 점프는 엄격하게 판단하는 게 좋습니다. 예를 들어
다음과 같은 조건들로 관리할 수 있습니다:
...
- `Positioning` vs `AI`: 내장 AI 상대로 방어는 잘 하지만 득점도 하지
  못할겁니다. 파워히트가 없어서 결국 막기밖에 못하는 코드입니다.
- `Positioning` vs `Minimal`: 완승. 뼈대만 있는 봇은 아예 못 움직이니까요.

## 다음 단계

방어는 되는데 공격 수단이 없습니다. 파워히트를 어떻게 얹을지는 여러분의
몫으로 남깁니다 → [Power Hit](power-hit.md).
